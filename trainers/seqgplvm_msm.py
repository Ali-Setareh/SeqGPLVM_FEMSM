...
seqgplvm_msm_from_py <- function(df,
                        train_ids,
                        propensity_scores_col = NULL,
                        k_last = 4,
                        a_val = NA_real_,
                        data_id = NA_character_,
                        training_id = NA_character_,
                        seed = NULL,
                        x_cols = NULL) {
  ...
  # Denominator seqgplvm: fit ONLY on variable-treatment units
  df_fe_FE <- df_fe[df_fe$patient_id %in% var_pids, , drop = FALSE]
  p_fe     <- clip01(df_fe_FE[[propensity_scores_col]])
