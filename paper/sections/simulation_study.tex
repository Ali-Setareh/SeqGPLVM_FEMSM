...
from the DGP described in Section 5.1 of \cite{blackwellEffectPoliticalAdvertising2024}. 
...
the same number of time points. In Eq.~\eqref{eq:blackwelldynamics}, we present 
the governing equations for the DGP. $\operatorname{expit}(x) = \frac{1}{1 + e^{-x}}$.

\begin{subequations}\label{eq:blackwelldynamics}
\begin{align}
X_{it} &\sim \mathcal{N}\!\left(-\frac{1}{2}\mathbbm{1},\, \Sigma\right), \qquad \Sigma_{jk} =
\begin{cases}
1, & j=k,\\
0.2, & j\neq k.
\end{cases}\\
U_i    &\sim \operatorname{Unif}(-a, a) \\
A_{it} &\sim \operatorname{Bernoulli}\!\left(\operatorname{expit}\!\left(U_i + \varphi A_{i,t-1} + \beta^{\top} X_{it}\right)\right) \\
\epsilon_{iT} &\sim \mathcal{N}(0,1) \\
Y_{iT} &= U_i + \tau_F D_{iT} + \tau_C \sum_{t=T-3}^{T-1} D_{it}
        + \gamma^{\top}\bar{X}_{iT} + \epsilon_{iT}
\end{align}
\end{subequations}
