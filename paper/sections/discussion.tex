...
\begin{itemize}
    ...
    \item A key limitation is that Hatt & Feuerriegel provide an identification argument 
    conditional on a correctly specified latent treatment model, 
    but they do not develop a corresponding asymptotic theory for the 
    resulting two-stage estimator. In particular, the paper does not analyze how 
    estimation (and approximation) error in the learned latent confounder—obtained from the 
    fitted CMM/SeqGPLVM—propagates into the downstream causal estimator (IPW/MSM or outcome regression), nor does it give conditions under which plug-in propensity scores lead to consistent, 
𝑁
N
	​

-asymptotically normal effect estimates with a valid variance estimator
\end{itemize}
